...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const users = await q`select * from admin_users where id = ${admin.id} and password_hash = ${currentHash}`;
    ...
    const partners = await q`select id, contact_details from ecosystem_partners where user_id = ${admin.id}`;
    ...
    const companies = await q`select id, contact_details from companies where user_id = ${admin.id}`;
